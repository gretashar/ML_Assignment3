...
from sklearn.metrics import silhouette_score as sil_score, homogeneity_score, accuracy_score
...
def kmeans_test(X,y, title):
    ...
    for i in range(2, 50):
        ...
        cs.append(sil_score(X, kmeans.labels_))
    ...
    print(labels)
    print(y)
    # accuracy=accuracy_score((1-kmeans.labels_), y)
    # # check how many of the samples were correctly labeled
    # correct_labels = sum(y == labels)

    # print("Result: %d out of %d samples were correctly labeled." % (accuracy, y.size))
    ...
    plt.grid()
    plt.savefig('./plots/'+title+' kmeans_sil_plot.png')  
    plt.clf()
    ...
    plt.ylabel('Time (s)')
    plt.grid()
    plt.savefig('./plots/'+title+' kmeans_time_plot.png')  
    plt.clf()
    ...
    # plt.title('The Elbow Method for '+title)
    # plt.xlabel('Number of clusters')
    ...
    km_model = KMeans(n_clusters=k, random_state=10)
    ...
    # initial_centers = random_center_initializer(X, k, random_state=5).initialize()
    # instance created for respective distance metric
    # instanceKm = kmeans(X, initial_centers=initial_centers, metric=distance_metric(0))
    # perform cluster analysis
    # instanceKm.process()
    # # cluster analysis results - clusters and centers
    # pyClusters = instanceKm.get_clusters()
...
y = df["output"]
kmeans_test(X, y, "Heart Failure Prediction")
...
X = pd.DataFrame(X, columns=[cols])
y = df["stroke"]
kmeans_test(X, y, "Stroke Prediction")